...
public class FormIdentification {
  ...
  public static Performable fillForm(String salaryType, boolean continueClick) {
    ...
    switch (salaryType) {
      case Constantes.TAG_VALID:
        salary = Constantes.VALID_SALARY;
        break;
      case Constantes.TAG_INVALID:
        salary = Constantes.INVALID_SALARY;
        break;
      case Constantes.TAG_PRO:
        salary = Constantes.PRO_SALARY;
        break;
      case Constantes.TAG_SIMPLE:
        salary = Constantes.SIMPLE_SALARY;
        break;
      case Constantes.TAG_EXCEED_MAX:
        salary = Constantes.EXCEED_SIZE_SALARY_TEXT;
        break;
      default:
        salary = "";
    }
  ...
  public static Performable fillAndContinue(String product) {
    ...
    return Task.where(
        ...
        Enter.theValue(actorData.getFirstName()).into(IdentificationPage.PRIMER_NOMBRE_INPUT),
        Enter.theValue(actorData.getLastName()).into(IdentificationPage.PRIMER_APELLIDO_INPUT),
        Check.whether(product.equals(Constantes.PRODUCT_CDA)).andIfSo(
            Enter.theValue(actorData.getSalary()).into(IdentificationPage.SALARY_INPUT)
            ),
        Click.on(IdentificationPage.HABEAS_DATA_CHECKBOX),
        Click.on(IdentificationPage.CONTINUE_BUTTON));
  ...
  public static Performable ingresarTextoFueraDelLimite() {
    return Task.where(
        ...
        Enter.theValue(Constantes.TEXTO_SUPERIOR_LIMITE_TAMANIO).into(IdentificationPage.PRIMER_NOMBRE_INPUT),
        Enter.theValue(Constantes.TEXTO_SUPERIOR_LIMITE_TAMANIO)
