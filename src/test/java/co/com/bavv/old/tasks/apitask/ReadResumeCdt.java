...
public class ReadResumeCdt implements Task {

    public static Performable get() {
        return instrumented(ReadResumeCdt.class);
    }

    @Override
    @Step("{0} lee informacion en base de datos para el resumen de cdt")
    public <T extends Actor> void performAs(T actor) {

        actor.whoCan(CallAnApi.at(ServicePaths.getEndPointBase(Constantes.DEV_ENV)));

        actor.attemptsTo(CallGetWith.token("read-resume-cdt"));
